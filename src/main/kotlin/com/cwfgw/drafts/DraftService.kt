...
class DraftService(
    ...
    suspend fun get(seasonId: SeasonId): Draft? = repository.findBySeason(seasonId)

    suspend fun create(
        seasonId: SeasonId,
        request: CreateDraftRequest,
    ): Draft = repository.create(seasonId, request)

    suspend fun start(seasonId: SeasonId): Result<Draft, DraftError> {
        val draft = repository.findBySeason(seasonId) ?: return Result.Err(DraftError.NotFound)
        if (draft.status != STATUS_PENDING) {
            return Result.Err(DraftError.WrongStatus(current = draft.status, expected = STATUS_PENDING))
        }
        val updated =
    ...
    suspend fun getPicks(seasonId: SeasonId): Result<List<DraftPick>, DraftError> {
        val draft = repository.findBySeason(seasonId) ?: return Result.Err(DraftError.NotFound)
        return Result.Ok(repository.getPicks(draft.id))
    }

    suspend fun getAvailableGolfers(seasonId: SeasonId): Result<List<Golfer>, DraftError> {
        val draft = repository.findBySeason(seasonId) ?: return Result.Err(DraftError.NotFound)
        return Result.Ok(repository.getAvailableGolfers(draft.id))
    }
    ...
    suspend fun initializePicks(
        ...
        if (teams.isEmpty()) return Result.Err(DraftError.NoTeams)
        val slots = snakeDraftOrder(teams.map { it.id }, rounds)
        return Result.Ok(repository.createPicks(draft.id, slots))
    }
    ...
    suspend fun makePick(
        ...
        if (draft.status != STATUS_IN_PROGRESS) {
            return Result.Err(DraftError.WrongStatus(current = draft.status, expected = STATUS_IN_PROGRESS))
        }
        val picks = repository.getPicks(draft.id)
        val next = picks.firstOrNull { it.golferId == null } ?: return Result.Err(DraftError.AllPicksMade)
        if (next.teamId != request.teamId) {
            return Result.Err(DraftError.NotYourTurn(actualTeam = next.teamId, requestedTeam = request.teamId))
        }
        val made =
